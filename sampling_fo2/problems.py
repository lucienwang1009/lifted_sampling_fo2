...
class MLNProblem(object):
    ...
    def __init__(self, rules: (list[(Rational, Rational)], list[Formula]),
                 domain: set[Const],
                 cardinality_constraint: CardinalityConstraint):
        self.rules = rules
...
def MLN_to_WFOMC(mln: MLNProblem):
    ...
    for weighting, formula in zip(*mln.rules):
        ...
        if weighting != float('inf'):
            aux_pred = new_predicate(len(formula.free_vars()), AUXILIARY_PRED_NAME)
            formula = Equivalence(formula, aux_pred(*formula.vars()))
