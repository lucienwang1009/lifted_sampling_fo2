...
from sampling_fo2.network.constraint import TreeConstraint, CardinalityConstraint
from sampling_fo2.fol.syntax import AndCNF, CNF, Const, Lit, Pred, Atom, \
    DisjunctiveClause, a, b, x, y
from sampling_fo2.fol.utils import new_predicate, exact_one_of
from sampling_fo2.utils import AUXILIARY_PRED_NAME, TSEITIN_PRED_NAME, \
        ...
        for flag in product(*([[True, False]] * len(ext_atoms))):
            ...
            if any(flag):
                for idx, f in enumerate(flag):
                    if not f:
                        continue
                    evidence = Lit(ext_atoms[idx], False)
                    ...
                    evidence_sentence.append(
                        CNF.from_formula(skolem_lit).Or(
                            CNF.from_formula(evidence)
                        )
                    )
                # skolem_pred = new_predicate(1, SKOLEM_PRED_NAME)
                # self.weights[skolem_pred] = (Rational(1, 1), Rational(-1, 1))
